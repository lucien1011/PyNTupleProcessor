...
from Data.BTagWorkingPoints import BTagWorkingPoints
import os
import csv
import pickle
import re
...
class WeightBTagSF(Module):
    ...
    """
    def __init__(self,name,applySystVariation=True,era="2016Moriond17",tagger = "CSV", btagDisc = 'CSVv2IVFM', opPoint = '1',forceSFToUnity=False,fileTag=""):
        super(WeightBTagSF,self).__init__(name)
    ...
    def beginJob(self,events,eventFunctions):
        ...
        if self._sample.isData:          return True
        if not self._PSet.reweightTrees: return True
        
        # Turn off reweighting for on-off comparison
        if not self._applySystVariation and self._PSet.systVar == "Up":       
            return True
        ...
        self.bsfFile = os.environ['ALPHATOOLSDIR']+"/Data/BTagging/"+self.era+"/btagScaleFactors"+self.fileTag+".p"
        ...
        elif "DM" in self._sample.name:
            self.effFile = os.environ['ALPHATOOLSDIR']+"/Data/BTagging/"+self.era+"/btagEfficiencies_DM.p"
        else:
            self.effFile = os.environ['ALPHATOOLSDIR']+"/Data/BTagging/"+self.era+"/btagEfficiencies_"+self._PSet.name.replace("_Up","").replace("_Down","")+".p"

        self.bsfDict = pickle.load( open(self.bsfFile,"rb") )
        #if not self._opPoint in self.bsfDict: raise ValueError("Operation point {0} not found in bsfDict".format(self._opPoint))
        self.effDict = pickle.load( open(self.effFile,"rb") )
        #if not self._opPoint in self.effDict['All'].keys(): raise ValueError("Operation point {0} not found in effDict".format(self._opPoint))
        self.bcfDict = pickle.load( open(self.bcfFile,"rb") ) if self.applyCF else None

        self.evalDict = self.makeEvalDict(self.bsfDict)

    @staticmethod
    def makeEvalDict(bsfDict):
        evalDict = {}
    ...
    def analyze(self,event,key=None):
        ...
        if not getattr(event,"jets",None): event.jets = PhysObjects(event,"jet")
        ...
        for jet in event.jets:
            ...
            # efficiency
            sampleKey = self.effDict.keys()[0] if self.applyCF else "All"
            if "DM" in self._sample.name: sampleKey = self._sample.name
