...
from RA5.Sequence.RecoSequence import tightlooseCR_sequence
...
plots = [
        Plot("nJet40",      ["TH1D","nJet40","",10,-0.5,9.5],       LambdaFunc('x: x.nJetRA540[0]')),
        Plot("nJet25",      ["TH1D","nJet25","",10,-0.5,9.5],       LambdaFunc('x: x.nJetRA525[0]')),
        Plot("nBJet40",     ["TH1D","nBJet40","",7,-0.5,6.5],      LambdaFunc('x: x.nBJetMediumRA540[0]')),
        Plot("nBJet25",     ["TH1D","nBJet25","",7,-0.5,6.5],      LambdaFunc('x: x.nBJetMediumRA525[0]')),
        Plot("nLepTight",    ["TH1D","nLepTight","",7,-0.5,6.5],      LambdaFunc('x: len(x.tightLeps)')),
        Plot("htJet",       ["TH1D","htJet","",20,0.,1000.],         LambdaFunc('x: x.htJet40[0]')),
        Plot("met_pt",      ["TH1D","met_pt","",25,0., 250.],          LambdaFunc('x: x.met_pt[0]')),
        Plot("met_phi",     ["TH1D","met_phi","",10,-5, 5.],          LambdaFunc('x: x.met_phi[0]')),
        Plot("mht",         ["TH1D","mht","",10,0., 500.],          LambdaFunc('x: x.mhtJet40[0]')),
                
        Plot("LepTightPt1",    ["TH1D","LepTightPt1","",20,0.,400.],      LambdaFunc('x: x.tightLeps[0].pt')),
        Plot("LepTightEta1",    ["TH1D","LepTightEta1","",10,-3.,3.],      LambdaFunc('x: x.tightLeps[0].eta')),
        Plot("LepTightPhi1",    ["TH1D","LepTightPhi1","",10,-5.,5.],      LambdaFunc('x: x.tightLeps[0].phi')),
        Plot("LepLoosePt2",    ["TH1D","LepTightPt2","",20,0.,200.],      LambdaFunc('x: x.looseLeps[0].pt')),
        Plot("LepLooseEta2",    ["TH1D","LepTightEta2","",10,-3.,3.],      LambdaFunc('x: x.looseLeps[0].eta')),
        Plot("LepLoosePhi2",    ["TH1D","LepTightPhi2","",10,-5.,5.],      LambdaFunc('x: x.looseLeps[0].phi')),

        Plot("JetPt1",    ["TH1D","JetPt1","",20,0.,400.],      LambdaFunc('x: x.selJets[0].pt')),
        Plot("JetEta1",    ["TH1D","JetEta1","",10,-3.,3.],      LambdaFunc('x: x.selJets[0].eta')),
        Plot("JetPhi1",    ["TH1D","JetPhi1","",10,-5.,5.],      LambdaFunc('x: x.selJets[0].phi')),
        Plot("JetPt2",    ["TH1D","JetPt2","",20,0.,400.],      LambdaFunc('x: x.selJets[1].pt')),
        Plot("JetEta2",    ["TH1D","JetEta2","",10,-3.,3.],      LambdaFunc('x: x.selJets[1].eta')),
        Plot("JetPhi2",    ["TH1D","JetPhi2","",10,-5.,5.],      LambdaFunc('x: x.selJets[1].phi')),
]
...
#for lepCat in lepCats:
    #plots.extend(
        ...
        #Plot("LepLoosePt2"+lepCat,    ["TH1D","LepTightPt2"+lepCat,"",20,0.,200.],      LambdaFunc('x: x.secondLep.pt')        ,selFunc=LambdaFunc('x: x.cat.lepCat == \"%s\"'%lepCat)),
        #Plot("LepLooseEta2"+lepCat,    ["TH1D","LepTightEta2"+lepCat,"",10,-3.,3.],      LambdaFunc('x: x.secondLep.eta')        ,selFunc=LambdaFunc('x: x.cat.lepCat == \"%s\"'%lepCat)),
        #Plot("LepLoosePhi2"+lepCat,    ["TH1D","LepTightPhi2"+lepCat,"",10,-5.,5.],      LambdaFunc('x: x.secondLep.phi')        ,selFunc=LambdaFunc('x: x.cat.lepCat == \"%s\"'%lepCat)),
...
sequence = tightlooseCR_sequence
