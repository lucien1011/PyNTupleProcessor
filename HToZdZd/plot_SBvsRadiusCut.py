...
signifFunclist    = ["Standard Signif."] # keys to a dictionary of significance functions
...
outputDir         = "/home/rosedj1/public_html/HToZdZd/SignificancePlots/AllKindsOfSignificance/SigBkgvsRadCuts/"
plotFileName      = "SBvsRadiusCut_mZdMASS_FINALSTATE" # Generic file name format. The `MASS' will become mZd value
linecolor         = 1 # 1=kBlack, starting line color
graphOptions      = "APC"
...
#                signifArr, signifErrArr = signifdict[signifFunc](sigArr, bkgArr, sigErrArr, bkgErrArr)
                ...
                tgSig = TGraphErrors(radius_cuts.size, radius_cuts, sigArr, radius_cutsErr, sigErrArr)
                tgBkg = TGraphErrors(radius_cuts.size, radius_cuts, bkgArr, radius_cutsErr, bkgErrArr)
                ...
                tgSig.SetLineColor(tmplinecolor)
                tgSig.SetLineWidth(2)
                mg.Add(tgSig)
                tmplinecolor+=1

                tgBkg.SetLineColor(tmplinecolor)
                tgBkg.SetLineWidth(2)
                mg.Add(tgSig)
                #tg, legentry = makeTGraph(massratio_cuts, signifArr, massratio_cutsErr, signifErrArr, finalstate, zdmass) 
                leg.AddEntry(tgSig,"Signal","lp") # l=line, p=point, f=fill
                leg.AddEntry(tgBkg,"Background","lp") # l=line, p=point, f=fill
#____________________________________________________________________________________________________
# Draw plots
            if (DRAW):
                plottitle   = "Number of Signal and Background Events, mZd = %s GeV, final state = %s," % (zdmass, finalstate)
                xtitle      = "Radius Cut [GeV]" 
                ytitle      = "Events / (%.2f) GeV" % binwid
