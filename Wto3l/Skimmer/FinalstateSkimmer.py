...
class FinalstateSkimmer(Module):
    ...
    def analyze(self,event):
        ...
        if ("Data_memCR_Run2016" in self.dataset.name) or ("WZTo3LNu_memCR" in self.dataset.name) or ("TTJets_memCR" in self.dataset.name) or ("DYJetsToLL_M50_memCR" in self.dataset.name) or ("DYJetsToLL_M10To50_memCR" in self.dataset.name):
            if abs(event.idL1[0]) == 11 and abs(event.idL2[0]) == 11 and abs(event.idL3[0]) == 13 and event.IsoL3[0] >= 0.35:# and event.etaL3[0] <= 1.4:
                return True
            elif abs(event.idL1[0]) == 11 and abs(event.idL2[0]) == 11 and abs(event.idL3[0]) == 13 and event.IsoL3[0] >= 0.35:# and event.etaL3[0] <= 1.4:
                return True
            else:
                return False
